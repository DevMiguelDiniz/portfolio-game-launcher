...
import { Trophy, Star, Code, Database, Smartphone, Globe, Calendar, TrendingUp, Award, Target, Zap, Crown } from 'lucide-react'
...
export default function AchievementsView() {
    ...
    const technologyStats = uniqueTechnologies.map(tech => {
    ...
    // Category statistics
    const categoryStats = [
        { name: 'Frontend', count: projects.filter(p => p.category === 'frontend').length, icon: <Globe className="w-5 h-5" />, color: 'text-blue-400' },
        { name: 'Backend', count: projects.filter(p => p.category === 'backend').length, icon: <Database className="w-5 h-5" />, color: 'text-green-400' },
        { name: 'Mobile', count: projects.filter(p => p.category === 'mobile').length, icon: <Smartphone className="w-5 h-5" />, color: 'text-purple-400' },
        { name: 'Full Stack', count: projects.filter(p => p.category === 'fullstack').length, icon: <Code className="w-5 h-5" />, color: 'text-orange-400' },
        { name: 'Web', count: projects.filter(p => p.category === 'web').length, icon: <Globe className="w-5 h-5" />, color: 'text-cyan-400' }
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="flex items-center space-x-4 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-gray-400 text-lg">Estatísticas detalhadas e marcos alcançados</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
                            <div className="text-2xl font-bold text-blue-400">{featuredProjects}</div>
                            <div className="text-gray-400 text-sm">Em Destaque</div>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
                            <div className="text-2xl font-bold text-green-400">{uniqueTechnologies.length}</div>
            ...
            <div className="p-8">
                <div className="max-w-6xl mx-auto space-y-12">
                    ...
                    <section>
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            {achievement.rarity}
                                            ...
                                            <p className={`text-sm mb-2 ${achievement.unlocked ? 'text-gray-300' : 'text-gray-500'}`}>
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <span className="text-gray-400 text-sm">{tech.usage} projetos</span>
                                                    <span className="text-blue-400 text-sm font-mono">{tech.percentage}%</span>
                                                </div>
                                            </div>
                                ...
                                >
                                    <div className="flex items-center space-x-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-400 text-sm">Projetos desenvolvidos</p>
                                        </div>
                                    </div>
                                        ...
                                        <div
                                            className={`h-2 rounded-full transition-all duration-1000 ${
                                                category.color.includes('blue') ? 'bg-blue-600' :
                                                    category.color.includes('green') ? 'bg-green-600' :
                                                        category.color.includes('purple') ? 'bg-purple-600' :
                                                            category.color.includes('orange') ? 'bg-orange-600' :
                                                                'bg-cyan-600'
                                            }`}
                    ...
                    <section className="bg-gray-900 rounded-lg p-8 border border-gray-800">
                        <h2 className="text-2xl font-bold text-white mb-6 text-center">Resumo de Progresso</h2>
                        <div className="grid md:grid-cols-3 gap-8 text-center">
                            <div>
                                <div className="text-4xl font-bold text-blue-400 mb-2">{unlockedAchievements.length}/{achievements.length}</div>
                                <div className="text-gray-400">Conquistas Desbloqueadas</div>
                            </div>
                            <div>
                                <div className="text-4xl font-bold text-green-400 mb-2">{uniqueTechnologies.length}</div>
                                <div className="text-gray-400">Tecnologias Dominadas</div>
                            </div>
                            <div>
                                <div className="text-4xl font-bold text-yellow-400 mb-2">{totalProjects}</div>
                                <div className="text-gray-400">Projetos Completados</div>
                            </div>
                        </div>
